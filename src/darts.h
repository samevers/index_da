#ifndef DARTS_0_32_H_
#define DARTS_0_32_H_
...
#define DARTS_THROW(msg) throw Darts_0_32::Details::Exception( \
  __FILE__ ":" DARTS_LINE_STR ": exception: " msg)

namespace Darts_0_32 {
...
class DoubleArrayImpl {
 public:
  ...
  inline std::size_t commonPrefixSearch(const key_type *key, U *results,
    ...
    template <class U>
  inline std::size_t visit(std::size_t length, std::size_t node_pos, U *results, std::size_t max_num_results) const {
	  unit_type unit = array_[node_pos];
	  node_pos ^= unit.offset();
	  size_t node_pos_bak = node_pos;
	  std::size_t num_results = 0;
	  if (unit.has_leaf())
	  {
		  if (max_num_results > 0)
		  {
			  set_result(results, static_cast<value_type>(
				  array_[node_pos].value()), length);
		  }
		  ++num_results;
	  }

	  for (int i = 0; i < 256; ++i) {
		  node_pos = node_pos_bak;
		  node_pos ^= static_cast<uchar_type>(i);
		  unit = array_[node_pos];
		  if (unit.label() == static_cast<uchar_type>(i)) {
			  if (num_results >= max_num_results)
				  return num_results;
			  num_results += visit(length+1, node_pos, results+num_results, max_num_results-num_results);
		  }
	  }
	  return num_results;
  }
  ...
  template <class U>
  inline std::size_t prefixSearch(const key_type *key, U *results,
      std::size_t max_num_results, std::size_t length = 0,
      std::size_t node_pos = 0) const;
...
template <typename A, typename B, typename T, typename C>
template <typename U>
inline std::size_t DoubleArrayImpl<A, B, T, C>::prefixSearch(const key_type *key, U *results,
      std::size_t max_num_results, std::size_t length,
  ...
  if (length != 0) {
    for (std::size_t i = 0; i < length; ++i) {
	  ...
	  node_pos_bak = node_pos;
      node_pos ^= unit.offset();
      if (unit.has_leaf() && i<(length-1)) {
        if (num_results < max_num_results) {
          set_result(&results[num_results], static_cast<value_type>(
              array_[node_pos].value()), i + 1);
        }
    ...
    for ( ; key[length] != '\0'; ++length) {
	  ...
	  node_pos_bak = node_pos;
      node_pos ^= unit.offset();
      if (unit.has_leaf() && key[length+1] != '\0') {
        if (num_results < max_num_results) {
          set_result(&results[num_results], static_cast<value_type>(
              array_[node_pos].value()), length + 1);
        }
    ...
    num_results += visit(length, node_pos_bak, results+num_results, max_num_results-num_results);
